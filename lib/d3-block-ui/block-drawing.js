...
export function renderBlockImage(block, blockGroup, svg, options = {}) {
  ...
  let x = horizontalPadding + getShapeInsets(shapeType, height).left;
...
export function renderPlaceholder(child, height, block, blockGroup, svg, childIndex, x, options = {}) {
  ...
  blockGroup.append("path")
    ...
    .attr("d", getShapePath(shapeType, x, y, placeholderWidth, placeholderHeight))
...
export function calculateWidth(block, svg) {
  ...
  let width = shapeInsets.left + shapeInsets.right;
...
function getShapeInsets(shapeType, height) {
  if (shapeType === "noun") {
    const inset = Math.min(height / 4, blockDrawingConstants.horizontalPadding);
    return { left: inset, right: inset };
  }
  if (shapeType === "det") {
    return {
      left: Math.min(height / 4, blockDrawingConstants.horizontalPadding),
      right: 0,
    };
  }
