...
export function renderPlaceholder(child, height, block, blockGroup, svg, childIndex, x, options = {}) {
  ...
  const renderedPlaceholderWidth = getPlaceholderWidth(child);
  ...
  const shapeOptions = shapeType === "prep"
    ? { shoulderX: x + renderedPlaceholderWidth * 0.26 }
    : undefined;
  blockGroup.append("path")
    ...
    .attr(
      ...
      getShapePath(
        ...
        x,
        y,
        renderedPlaceholderWidth,
        renderedPlaceholderHeight,
        shapeOptions,
      ),
    )
    .attr("fill", inputColor);

  return renderedPlaceholderWidth + horizontalPadding;
}
...
export function calculateWidth(block, svg) {
  ...
  children.forEach((child) => {
    if (child.type === "placeholder") {
      width += child.content ? calculateWidth(child.content, svg) : getPlaceholderWidth(child);
    } else if (child.type === "text") {
...
export function getShapePath(shapeType, x, y, width, height, options = {}) {
  ...
  if (shapeType === "prep") {
    const { placeholderHeight } = blockDrawingConstants;
    ...
    const rightRadius = Math.min(height / 2, right - shoulderX);
    const slotRadius = Math.min(placeholderHeight / 2, rightRadius);
    const leftArcCenterX = shoulderX + slotRadius;
    const verticalOffset = stemHeight / 2;
    const leftJoinOffset = Math.sqrt(
      Math.max(0, rightRadius ** 2 - verticalOffset ** 2),
    );
    const leftJoinX = leftArcCenterX - leftJoinOffset;

    return [
      `M ${x + cornerRadius} ${stemTop}`,
      `H ${leftJoinX}`,
      `A ${rightRadius} ${rightRadius} 0 0 1 ${leftArcCenterX} ${y}`,
      `H ${right - rightRadius}`,
      `A ${rightRadius} ${rightRadius} 0 0 1 ${right - rightRadius} ${bottom}`,
      `H ${leftArcCenterX}`,
      `A ${rightRadius} ${rightRadius} 0 0 1 ${leftJoinX} ${stemBottom}`,
      `H ${x + cornerRadius}`,
...
function getPlaceholderWidth(child) {
  return child.expectedHeadType === "prep"
    ? blockDrawingConstants.prepPlaceholderWidth
    : blockDrawingConstants.placeholderWidth;
}
...
function calculatePrepShoulderX(block, svg) {
  ...
  for (const child of block.children) {
    ...
    if (child.type === "placeholder") {
      x += child.content ? calculateWidth(child.content, svg) : getPlaceholderWidth(child);
    } else if (child.type === "text") {
